...
class App extends Component {
  ...
  onChangeSearchDestination = searchInput1 => {
    this.setState({searchInput: searchInput1})
  }

  render() {
    const {searchInput} = this.state
    const changedList = destinationsList.filter(eachItem =>
      eachItem.name.toLowerCase().includes(searchInput.toLowerCase()),
    )

    return (
      <DestinationSearch
        destinationsList={changedList}
        onChangeValue={this.onChangeSearchDestination}
      />
    )
  }
}
